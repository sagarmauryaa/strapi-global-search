...
const EMPTY = { results: [], groups: [], pagination: { page: 1, pageSize: 20, total: 0, pageCount: 0 }, meta: {} };
 ...
 */
const useGlobalSearch = ({ debounce = 250, minChars = 2, pageSize = 20 } = {}) => {
  const [query, setQuery] = useState('');
  const [selectedTypes, setSelectedTypes] = useState([]);
  const [locale, setLocale] = useState('all');
  const [page, setPage] = useState(1);
  const [state, setState] = useState({ status: 'idle', data: EMPTY, error: null });
  ...
  const toggleType = useCallback((uid) => {
